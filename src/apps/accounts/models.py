...
from apps.novels.models import Novel, Chapter, Like, Comment

# Exceptions
from apps.novels.exceptions import (
...
# Decorators
from .decorators import must_be_the_author, must_be_public, notify_author, notify_readers
...
class StallionUser(AbstractUser):
    ...
    gender = models.CharField(max_length=256, choices=GENDERS, default="Homme", blank=True)
    ...
    def comment_chapter(self, chapter_id, message):
        chapter = Chapter.objects.get(pk=chapter_id)
        comment = Comment.objects.create(author=self,chapter=chapter,content=message)
        return comment
