...
from ..exceptions import (
    NotYourNovelException,
    AlreadyInLibraryException,
    IsNotPublicException,
    AlreadyLikedException,
    NotLikedYetException,
    NotInLibraryException,
)
...
class NotificationTestCase(TestCase):
    ...
    def test_author_is_notified_when_chapter_is_liked(self):
        ...
        chapter = self.john.create_chapter(novel.id, title="Chapter", public=True)
        self.ismael.like_chapter(chapter.id)
        self.assertTrue(
            Notification.objects.filter(
                recipient=chapter.novel.author,
                verb=f"{self.ismael.username} aime votre chapitre {chapter.title}",
            ).exists()
        )
    ...
    def test_author_is_notified_when_chapter_is_unliked(self):
        ...
        self.assertTrue(
            Notification.objects.filter(
                recipient=chapter.novel.author,
                verb=f"{self.ismael.username} n'aime plus votre chapitre {chapter.title}",
            ).exists()
        )
    ...
    def test_author_is_notified_when_chapter_commented(self):
        ...
        self.assertTrue(
            Notification.objects.filter(
                recipient=chapter.novel.author,
                verb=f"{self.ismael.username} a commente votre chapitre {chapter.title}",
            ).exists()
        )
    ...
    def test_user_is_notified_when_library_novel_is_updated(self):
        ...
        self.assertTrue(
            Notification.objects.filter(
                recipient=self.ismael,
                verb=f"L'auteur {self.john.username} a modifie les informations sur son livre {novel.title}",
            ).exists()
        )
    ...
    def test_user_is_notified_when_library_novel_is_deleted(self):
        ...
        self.assertTrue(
            Notification.objects.filter(
                recipient=self.ismael,
                verb=f"L'auteur {self.john.username} a supprime son livre {novel.title}",
            ).exists()
        )
    ...
    def test_user_is_notified_when_library_novel_is_republished(self):
        ...
        self.assertTrue(
            Notification.objects.filter(
                recipient=self.ismael,
                verb=f"L'auteur {self.john.username} a rendu public son livre {novel.title}",
            ).exists()
        )
    ...
    def test_user_is_notified_when_library_novel_is_unpublished(self):
        ...
        self.assertTrue(
            Notification.objects.filter(
                recipient=self.ismael,
                verb=f"L'auteur {self.john.username} a rendu non-public son livre {novel.title}",
            ).exists()
        )
    ...
    def test_user_is_notified_when_library_novel_chapter_is_updated(self):
        ...
        self.assertTrue(
            Notification.objects.filter(
                recipient=self.ismael,
                verb=f"L'auteur {self.john.username} a modifie les informations sur le chapitre {chapter.title} de son livre {novel.title}",
            ).exists()
        )
    ...
    def test_user_is_notified_when_library_novel_chapter_is_deleted(self):
        ...
        self.assertTrue(
            Notification.objects.filter(
                recipient=self.ismael,
                verb=f"L'auteur {self.john.username} a supprime le chapitre {chapter.title} de son livre {novel.title}",
            ).exists()
        )
    ...
    def test_user_is_notified_when_library_novel_chapter_is_published(self):
        ...
        self.assertTrue(
            Notification.objects.filter(
                recipient=self.ismael,
                verb=f"L'auteur {self.john.username} a rendu public le chapitre {chapter2.title} de son livre {novel.title}",
            ).exists()
        )
    ...
    def test_user_is_notified_when_library_novel_chapter_is_unpublished(self):
        ...
        self.assertTrue(
            Notification.objects.filter(
                recipient=self.ismael,
                verb=f"L'auteur {self.john.username} a rendu non-public le chapitre {chapter.title} de son livre {novel.title}",
            ).exists()
        )
    ...
    def test_user_not_notified_when_unaccessible_chapter_is_updated(self):
        novel = self.john.create_novel(title="Novel", public=True)
        chapter = self.john.create_chapter(novel.id, title="Chapter", public=True)
        chapter2 = self.john.create_chapter(novel.id, title="Chapter", public=True)
        self.ismael.add_to_library(novel.id)
...
class PublishingTestCase(TestCase):
    ...
    def test_novel_is_published(self):
        novel = self.john.publish_novel(self.john.create_novel(title="Johnas").id)
    ...
    def test_cannot_publish_or_unpublish_other_people_chapter(self):
        ...
        chapter = self.john.create_chapter(johns_novel.id, title="Chapter", public=True)
        with self.assertRaises(NotYourNovelException):
...
class ChapterTestCase(TestCase):
    ...
    def test_like_chapter(self):
        ...
        self.assertTrue(
            Like.objects.filter(chapter=johns_chapter, liker=ismael).exists()
        )
    ...
    def test_cannot_like_not_public_chapter(self):
        ...
        johns_novel = Novel.objects.get(title="Johnas")
        johns_novel = john.publish_novel(johns_novel.id)
        johns_chapter = john.create_chapter(johns_novel.id, title="Chapter John")
...
class LibraryTestCase(TestCase):
    ...
    def test_can_add_novel_to_library(self):
        johns_novel = self.john.create_novel(title="The Book of John", public=True)
